...
export class LightStore extends AbstractStore {
  async update() {
    ...
    if (currentCountry) {
      ...
      if (allItems) {
        const onlyWhitelisted = await this.onlyWhitelistedItems(
          allItems,
          whitelist
        );
      ...
      if (this.isRoot) {
        const regionalFolder = await db.getRegionalFolderForCountry(
          currentCountry
        );
  ...
  onlyWhitelistedItems = async (
    driveItems: Array<IDriveItem>,
    whitelistUrls: string[]
  ): Promise<Array<IDriveItem>> => {
